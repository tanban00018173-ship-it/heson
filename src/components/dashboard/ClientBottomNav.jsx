  ...
  return (
    ...
    >
      <div className="flex">
        ...
        >
          {(location.pathname === '/ClientProfile' || location.pathname.includes('ClientProfile')) && (
            <span className="absolute top-0 left-1/2 -translate-x-1/2 w-8 h-0.5 bg-stone-900 rounded-full" />
          )}
          <User
            className={`w-5 h-5 transition-colors ${location.pathname.includes('ClientProfile') ? 'text-stone-900' : 'text-stone-300'}`}
            strokeWidth={location.pathname.includes('ClientProfile') ? 2.5 : 1.8}
          />
          <span className={`text-[10px] font-medium transition-colors ${location.pathname.includes('ClientProfile') ? 'text-stone-900' : 'text-stone-300'}`}>
            我的
          </span>
        </button>
      </div>
