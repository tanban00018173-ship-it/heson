...
import { MapPin, ChevronDown, Bell, X, PlusCircle, Home, Building2, HelpCircle, ChevronRight, Check, Loader2 } from 'lucide-react';
    ...
    <div className="fixed inset-0 z-[100] flex flex-col justify-end">
      ...
      <div className="relative bg-white rounded-t-3xl px-5 pt-5 pb-8 flex flex-col gap-4 animate-in slide-in-from-bottom duration-200">
        ...
        <input
          ...
          className="w-full border border-stone-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-stone-300"
          autoFocus
        />
        <button
          onClick={() => onSave(draft)}
          className="w-full bg-stone-900 text-white font-bold py-3.5 rounded-2xl"
        >
          儲存
        </button>
      </div>
...
export default function AddressBar({ onAddressChange }) {
  const navigate = useNavigate();
  const [user, setUser] = useState(null);
  const [selectedAddrObj, setSelectedAddrObj] = useState(null);
  const [addressText, setAddressText] = useState('台北市・居家服務');
  ...
  useEffect(() => {
    base44.auth.isAuthenticated().then(ok => {
      ...
      base44.auth.me().then(async u => {
        ...
        if (def) applyAddress(def, false);
      });
    });
  }, []);

  const applyAddress = (addrObj, close = true) => {
    setSelectedAddrObj(addrObj);
    const text = `${addrObj.district || addrObj.city}・${addrObj.street?.slice(0, 8) || '居家服務'}`;
    setAddressText(text);
    if (onAddressChange) onAddressChange(addrObj);
    ...
    <>
      <div className="flex items-center justify-between px-4 py-3 bg-white">
        ...
        >
          ...
          <span className="text-sm font-semibold text-stone-800 truncate">{addressText}</span>
          ...
          <motion.div className="fixed inset-0 z-50 flex flex-col justify-end" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            ...
            >
              <div className="px-5 pt-5 pb-3 flex items-center justify-between border-b border-stone-100">
                <h2 className="text-base font-bold text-stone-900">選擇服務地址</h2>
                <button onClick={() => setShowPicker(false)} className="w-8 h-8 bg-stone-100 rounded-full flex items-center justify-center">
                  <X className="w-4 h-4 text-stone-600" />
                </button>
              </div>
              <div className="px-5 pt-3 space-y-2 max-h-72 overflow-y-auto">
                <button
                  onClick={() => { setShowNewAddr(true); setNewAddrStep('form'); }}
                  className="w-full flex items-center gap-3 p-3.5 rounded-2xl border-2 border-dashed border-stone-200 hover:border-stone-400 transition-all text-left"
                >
                  <div className="w-9 h-9 bg-stone-50 rounded-xl flex items-center justify-center flex-shrink-0">
                    <PlusCircle className="w-5 h-5 text-stone-500" />
                  </div>
                  <p className="text-sm text-stone-600 font-medium">新增地址</p>
                </button>
                {savedAddresses.map(addr => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-start gap-3 p-3.5 rounded-2xl border transition-all text-left ${isSelected ? 'border-stone-900 bg-stone-50' : 'border-stone-100 hover:border-stone-300 bg-white'}`}
                    >
                      <div className="w-9 h-9 bg-stone-100 rounded-xl flex items-center justify-center flex-shrink-0 mt-0.5">
                        <Icon className="w-4 h-4 text-stone-600" />
                      </div>
                      <div className="min-w-0 flex-1">
                        <p className="text-xs text-stone-400 mb-0.5">{addr.address_type}{addr.is_default ? ' · 預設' : ''}</p>
                        <p className="text-sm text-stone-900 font-medium truncate">{addr.full_name}</p>
                        <p className="text-xs text-stone-400 truncate">{fullAddr}</p>
                      </div>
                      {isSelected && <Check className="w-4 h-4 text-stone-900 flex-shrink-0 mt-1" />}
                    </button>
                  );
                })}
              </div>
      ...
      <AnimatePresence>
        {showNewAddr && (
          <motion.div className="fixed inset-0 z-[60] flex flex-col justify-end" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <div className="absolute inset-0 bg-black/60" onClick={() => setShowNewAddr(false)} />
            <motion.div
              ...
              transition={{ type: 'spring', damping: 30, stiffness: 300 }}
            >
              <div className="px-5 pt-5 pb-3 flex items-center justify-between border-b border-stone-100 flex-shrink-0">
                <h2 className="text-base font-bold text-stone-900">新增服務地址</h2>
              ...
              <div className="overflow-y-auto flex-1 pb-6">
                <p className="px-5 py-3 text-xs text-stone-400 font-semibold uppercase tracking-wider">地址類型</p>
                <div className="px-5 flex gap-2 flex-wrap mb-2">
                  {CLEANING_TYPES.map(type => (
                    <button key={type} onClick={() => setNewAddr(f => ({ ...f, address_type: type }))}
                      className={`px-4 py-2 rounded-full text-sm font-medium border transition-colors ${newAddr.address_type === type ? 'bg-stone-900 text-white border-stone-900' : 'bg-white text-stone-600 border-stone-200 hover:border-stone-400'}`}>
                      {type}
                    </button>
                  ))}
                </div>

                <p className="px-5 py-3 text-xs text-stone-400 font-semibold uppercase tracking-wider">聯絡人資訊</p>
                <div className="bg-stone-50 mx-5 rounded-2xl overflow-hidden mb-2">
                  <button onClick={() => setEditField({ key: 'full_name', title: '全名', value: newAddr.full_name, inputType: 'text', placeholder: '請輸入全名' })}
                    ...
                    <div className="flex items-center gap-1.5">
                      <span className="text-sm font-medium text-stone-900">{newAddr.full_name || '—'}</span>
                  ...
                  <button onClick={() => setEditField({ key: 'phone', title: '手機號碼', value: newAddr.phone, inputType: 'tel', placeholder: '請輸入手機號碼' })}
                    ...
                    <div className="flex items-center gap-1.5">
                      <span className="text-sm font-medium text-stone-900">{newAddr.phone || '—'}</span>
                      <ChevronRight className="w-4 h-4 text-stone-400" />
                    </div>
                  </button>
                </div>

                <p className="px-5 py-3 text-xs text-stone-400 font-semibold uppercase tracking-wider">地址資訊</p>
                <div className="bg-stone-50 mx-5 rounded-2xl overflow-hidden mb-4">
                  <button onClick={() => setNewAddrStep('region')}
                    className="w-full flex items-center justify-between px-4 py-3.5 border-b border-stone-100 hover:bg-stone-100 transition-colors">
                    <span className="text-sm text-stone-500">城市／區</span>
                    <div className="flex items-center gap-1.5">
                      <span className="text-sm font-medium text-stone-900">{newAddr.city && newAddr.district ? `${newAddr.city} ${newAddr.district}` : '—'}</span>
                      <ChevronRight className="w-4 h-4 text-stone-400" />
                    </div>
                  </button>
                  ...
                  <button onClick={() => setNewAddrStep('street')}
                    ...
                    <div className="flex items-center gap-1.5">
                      <span className="text-sm font-medium text-stone-900 max-w-[160px] truncate text-right">{newAddr.street || '—'}</span>
                      <ChevronRight className="w-4 h-4 text-stone-400" />
                    </div>
                  </button>
                </div>
                ...
                <div className="px-5">
                  <button
                    ...
                    className="w-full bg-stone-900 text-white font-bold py-4 rounded-2xl disabled:opacity-40 transition-colors flex items-center justify-center gap-2"
                  >
