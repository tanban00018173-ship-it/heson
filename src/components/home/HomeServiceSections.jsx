...
/* ─── 卡片組件（2欄 grid 樣式） ─── */
function DbCard({ item, onTrack, large = false }) {
  ...
  return (
    <button onClick={() => {
      ...
      className="w-full rounded-2xl overflow-hidden text-left active:scale-[0.98] transition-transform border border-stone-100 shadow-sm bg-white"
    >
      <div className={`${large ? 'h-36' : 'h-28'} bg-stone-100 flex items-center justify-center relative overflow-hidden`}>
        {item.image_url
          ? <img src={item.image_url} alt={item.title} className="w-full h-full object-cover" />
          : <span className="text-4xl">{item.emoji || '🧹'}</span>}
        {item.badge && <span className="absolute top-2 left-2 bg-red-500 text-white text-[9px] font-bold px-1.5 py-0.5 rounded-full">{item.badge}</span>}
      </div>
      <div className="p-2.5">
        <p className={`font-bold text-stone-800 leading-tight truncate ${large ? 'text-sm' : 'text-xs'}`}>{item.title}</p>
        <p className="text-[10px] text-stone-400 mt-0.5 leading-tight line-clamp-1">{item.subtitle}</p>
        {item.price && <p className={`font-bold text-stone-900 mt-1 ${large ? 'text-sm' : 'text-xs'}`}>NT$ {item.price.toLocaleString()} 起</p>}
      </div>
    </button>
  );
}

function CleanerCard({ profile, reviews = [], onTrack, large = false }) {
  const navigate = useNavigate();
  ...
  return (
    ...
    >
      <div className={`relative ${large ? 'h-36' : 'h-28'} bg-gradient-to-br from-stone-100 to-stone-200`}>
      ...
      <div className="p-2.5">
        <p className={`font-semibold text-stone-800 truncate ${large ? 'text-sm' : 'text-xs'}`}>{profile.nickname || '管理師'}</p>
...
function ProductCard({ product, onTrack, large = false }) {
  const navigate = useNavigate();
  return (
    <button onClick={() => { onTrack('click_card', { section_key: 'shop', target_id: product.id, target_name: product.name }); navigate('/ClientShop'); }}
      className="w-full bg-white rounded-2xl overflow-hidden shadow-sm border border-stone-100 text-left active:scale-[0.98] transition-transform"
    >
      <div className={`${large ? 'h-36' : 'h-28'} bg-stone-100 flex items-center justify-center overflow-hidden`}>
      ...
      <div className="p-2.5">
        <p className={`font-bold text-stone-900 ${large ? 'text-sm' : 'text-xs'}`}>NT$ {product.price?.toLocaleString()}</p>
        <p className="text-[10px] text-stone-600 mt-0.5 leading-tight line-clamp-2">{product.name}</p>
      </div>
    </button>
  );
}
...
function PlaceholderGrid() {
  return (
    <div className="grid gap-3" style={{ gridTemplateColumns: '3fr 2fr' }}>
      <div className="rounded-2xl overflow-hidden border border-dashed border-stone-200 bg-stone-50">
        <div className="h-36 flex items-center justify-center text-stone-200 text-3xl">🧹</div>
        <div className="p-2.5"><div className="h-3 bg-stone-200 rounded-full w-3/4 mb-2 animate-pulse" /><div className="h-2.5 bg-stone-100 rounded-full w-1/2 animate-pulse" /></div>
      </div>
      <div className="rounded-2xl overflow-hidden border border-dashed border-stone-200 bg-stone-50">
        <div className="h-28 flex items-center justify-center text-stone-200 text-3xl">🧹</div>
        <div className="p-2.5"><div className="h-3 bg-stone-200 rounded-full w-3/4 mb-2 animate-pulse" /><div className="h-2.5 bg-stone-100 rounded-full w-1/2 animate-pulse" /></div>
      </div>
    </div>
  );
...
function SectionRow({ def, items, cleaners, reviews, products, onTrack }) {
  ...
  const renderGrid = (cards) => {
    if (cards.length === 0) return <PlaceholderGrid />;
    // 每兩個為一組：左大右小
    const pairs = [];
    for (let i = 0; i < Math.min(cards.length, 6); i += 2) {
      pairs.push(cards.slice(i, i + 2));
    }
    return (
      <div className="flex flex-col gap-3">
        {pairs.map((pair, pi) => (
          <div key={pi} className="grid gap-3" style={{ gridTemplateColumns: '3fr 2fr' }}>
            <div>{pair[0]}</div>
            {pair[1] ? <div>{pair[1]}</div> : <div />}
          </div>
        ))}
      </div>
    );
  ...
  const getCards = () => {
    if (def.key === 'featured_cleaners') {
      return cleaners.slice(0, 6).map((p, i) => (
        <CleanerCard key={p.id} profile={p} reviews={reviews.filter(r => r.cleaner_id === p.user_id)} onTrack={onTrack} large={i % 2 === 0} />
      ));
    }
    if (def.key === 'shop') {
      return products.slice(0, 6).map((p, i) => (
        <ProductCard key={p.id} product={p} onTrack={onTrack} large={i % 2 === 0} />
      ));
    }
    return items.slice(0, 6).map((item, i) => (
      <DbCard key={item.id} item={item} onTrack={onTrack} large={i % 2 === 0} />
    ));
  };

  return (
    <section className="bg-white mt-2 pt-4 pb-4">
      <div className="flex items-center justify-between px-4 mb-3">
        <div className="flex items-center gap-2">
          <p className="text-base font-black text-stone-900">{def.title}</p>
          {def.badge && <span className="text-[10px] font-bold px-1.5 py-0.5 bg-red-500 text-white rounded-full">{def.badge}</span>}
        </div>
        <button onClick={() => navigate(def.ctaPath)}
