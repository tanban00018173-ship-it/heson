  ...
  return (
    ...
    >
      ...
      <div className="p-3 flex gap-2.5">
        {providerPhoto ? (
          <img src={providerPhoto} alt="" className="w-14 h-14 rounded-full object-cover border border-stone-100 flex-shrink-0" />
        ) : (
        ...
        <div className="flex-1 min-w-0">
          <p className="font-black text-base text-stone-900 leading-tight line-clamp-2">{item.title}</p>
          {item.subtitle && <p className="text-xs text-stone-400 mt-0.5 leading-snug line-clamp-1">{item.subtitle}</p>}
          {item.price && <p className="text-sm font-black text-stone-900 mt-1.5">NT$ {item.price.toLocaleString()} 起</p>}
        </div>
      </div>
    </button>
  );
