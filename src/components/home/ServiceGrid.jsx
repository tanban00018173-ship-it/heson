...
const SERVICES = [
  { id: 'svc_home',    name: '居家清潔', sub: '鐘點・定期・大掃除', Icon: Home,      price: 2800, tag: '最熱門', tagColor: 'bg-amber-500' },
        ...
        {SERVICES.map((svc) => {
          ...
          return (
            ...
            >
              ...
              <div className={`w-11 h-11 rounded-2xl flex items-center justify-center mb-3 ${added ? 'bg-amber-100' : 'bg-stone-50 border border-stone-100'}`}>
                <svc.Icon className={`w-5 h-5 ${added ? 'text-amber-600' : 'text-stone-500'}`} />
              </div>
              <p className={`text-sm font-bold leading-tight ${added ? 'text-amber-800' : 'text-stone-800'}`}>
                {svc.name}
              </p>
              ...
              <div className="mt-2 flex items-baseline gap-1">
                ...
                <span className={`text-sm font-black ${added ? 'text-amber-600' : 'text-stone-700'}`}>
