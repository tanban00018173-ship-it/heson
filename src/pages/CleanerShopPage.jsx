...
function getLevel(years) {
  if (!years || years < 2) return { label: '清潔師傅', bgColor: 'bg-stone-400', icon: '🧹' };
  if (years < 5) return { label: '清潔達人', bgColor: 'bg-orange-500', icon: '⭐' };
  return { label: '清潔大師', bgColor: 'bg-purple-600', icon: '👑' };
}
        ...
        <div className="relative z-10 flex items-center gap-3 px-3 py-4">
          ...
          <div className="relative flex-shrink-0">
            ...
            {(() => {
              ...
              return (
                <span className={`absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-1/2 ${lv.bgColor} text-white text-[9px] font-bold px-2 py-0.75 rounded-full shadow whitespace-nowrap`}>
                  {lv.label}
                </span>
              );
            })()}
