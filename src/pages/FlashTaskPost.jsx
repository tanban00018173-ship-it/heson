...
import { Zap, MapPin, Loader2, Check, ArrowLeft, Plus, Minus, UtensilsCrossed, Trash2, WashingMachine, Package, RefreshCw, AlertTriangle, Home, Building2, HelpCircle, ChevronDown, PlusCircle, X, ChevronRight } from 'lucide-react';
...
export default function FlashTaskPost() {
  ...
  const handleSaveNewAddr = async () => {
    ...
    try {
      ...
      const fullAddr = `${saved.city}${saved.district}${saved.street}`;
      setAddress(fullAddr);
      if (saved.gps_lat) setGpsLat(saved.gps_lat);
      if (saved.gps_lng) setGpsLng(saved.gps_lng);
